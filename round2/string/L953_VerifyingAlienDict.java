 ...
 * .
 */
public class L953_VerifyingAlienDict {
    int[] map;
    public boolean isAlienSorted(String[] words, String order) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            if (compareAlien(word1, word2) > 0) return false;
        }
        return true;
    }

    private int compareAlien(String s, String t) {
        int i = 0;
        int j = 0;
        while (i < s.length() && j < t.length()) {
            if (map[s.charAt(i) - 'a'] == map[t.charAt(j) - 'a']) {
                i++;
                j++;
            } else if (map[s.charAt(i) - 'a'] > map[t.charAt(j) - 'a']) {
                return 1;
            } else {
                return -1;
            }
