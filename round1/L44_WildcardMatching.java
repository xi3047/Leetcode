...
/*
    ...
    Given an input round1_misc.string (s) and a pattern (p), implement wildcard pattern matching with support for '?' and '*'.

'?' Matches any single character.
'*' Matches any sequence of characters (including the empty sequence).
The matching should cover the entire input round1_misc.string (not partial).
...
p = "a"
Output: false
Explanation: "a" does not match the entire round1_misc.string "aa".
Example 2:
...
public class L44_WildcardMatching {
    ...
    public boolean isMatch_2d_method(String s, String p) {
        ...
        // set initial match to true since empty pattern matches with empty round1_misc.string
        dp[0][0] = true;
...
//        }
        ...
        //if the first character in the pattern is *, then it matches the empty round1_misc.string, assuming there coule be multiple leading *s
        for(int j=1; j<=n; j++) {
            if(p.charAt(j-1)=='*'){
                dp[0][j] = true;
            } else {
        ...
        for(int i=1; i<=m; i++) {
            for(int j=1; j<=n; j++) {
                if (p.charAt(j-1) == '?' || s.charAt(i-1) == p.charAt(j-1)) { // when last char in the pattern is either '?' or it is the same as the last character in the text
                    ...
                    // the last char in the round1_misc.string as a part of '*' sequences
                }
            }
        }
